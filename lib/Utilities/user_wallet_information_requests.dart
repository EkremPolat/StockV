...
Future<List<WalletCoin>> getUserWallet(String userID) async {
  var url = 'http://172.20.10.2:8000/get-wallet/$userID/';
...
Future<double> buyCrypto(
  ...
  var url = 'http://172.20.10.2:8000/buy-crypto/';
...
Future<HasCryptoData> hasCrypto(String userId, String coinName) async {
  const url = 'http://172.20.10.2:8000/has-crypto/';
...
Future<double> sellCrypto(
  ...
  var url = 'http://172.20.10.2:8000/sell-crypto/';
  var response = await http.post(Uri.parse(url),
      body:
...
Future<List<Transaction>> getUserTransactionHistory (String userId) async {
  var url = 'http://172.20.10.2:8000/get-transaction-history/$userId/';
...
Future<double> getUserBalance(String userId) async {
  var url = 'http://172.20.10.2:8000/get-user-balance/$userId/';
  var response = await http.get(Uri.parse(url));
  if (response.statusCode == 200) {
