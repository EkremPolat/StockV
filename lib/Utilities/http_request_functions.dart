...
Future<bool> register(String fullName, String email, String password) async {
  var url = 'http://172.20.10.2:8000/signup/';
...
Future<User?> login(String email, String password) async {
  var url = 'http://172.20.10.2:8000/login/';
  var response = await http.post(
    Uri.parse(url),
...
Future<String?> passwordChange(String? fullName, String? email,
  ...
  var passwordVerificationURL = 'http://172.20.10.2:8000/verify-password/';
  ...
  if (passwordVerificationResponse.statusCode == 200) {
    var url = 'http://172.20.10.2:8000/password-change/';
    dynamic response;
...
Future<List<Coin>> fetchCoinsFromDB() async {
  var url = 'http://172.20.10.2:8000/coins/';
...
Future<List<Coin>> fetchSavedCoinsFromDB(String userID) async {
  var url = 'http://172.20.10.2:8000/saved-coins/$userID/';
  var response = await http.get(
    Uri.parse(url),
  );
  if (response.statusCode == 200) {
...
Future<List<Coin>> addSavedCoinsToUser(String userId, int coinId) async {
  var url = 'http://172.20.10.2:8000/add-saved-coin/';
  var response = await http.post(
    Uri.parse(url),
...
Future<List<Coin>> removeSavedCoinsFromUser(String userId, int coinId) async {
  var url = 'http://172.20.10.2:8000/remove-saved-coin/';
  var response = await http.post(
    Uri.parse(url),
    body: json.encode({'userId': userId, 'coinId': coinId}),
    headers: {
